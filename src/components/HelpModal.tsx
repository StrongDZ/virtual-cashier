...
const HelpModal = ({ onClose }: HelpModalProps) => {
    ...
    const generateBotResponse = (question: string): string => {
        ...
        if (
            lowerQuestion.includes("catalogue") ||
            lowerQuestion.includes("catalog") ||
    ...
    const quickQuestions = ["Where is the scanner?", "How do I pay?", "Can I return items?", "How to use voice commands?"];
            ...
            >
                            ...
                            >
                                <div
                                    className={`max-w-[80%] rounded-2xl p-4 ${
                                        msg.sender === "user" ? "bg-gradient-to-r from-accent to-accent-dark text-slate-900" : "glass text-white"
                                    }`}
                                >
                    ...
                    >
                        {quickQuestions.map((question, index) => (
                            <motion.button
                                ...
                                className="flex-shrink-0 px-4 py-2 glass rounded-full text-sm text-gray-300 hover:text-white hover:bg-white/20 transition-all whitespace-nowrap"
                ...
                <div className="p-4 border-t-2 border-accent/30">
                    <div className="flex items-center gap-3">
                        ...
                        {interactionMode !== "touch" && isSupported && (
                            <motion.button
                                ...
                                className={`p-3 rounded-xl transition-all ${
                                    isListening ? "bg-red-500 text-white" : "glass text-gray-400 hover:text-white hover:bg-white/20"
                                }`}
                                whileHover={{ scale: 1.05 }}
                                whileTap={{ scale: 0.95 }}
                            >
                        ...
                        {/* Send Button */}
                        <Button variant="primary" onClick={handleSendMessage} disabled={!inputText.trim() || isProcessing} className="px-4">
                            <Send size={24} />
                        </Button>
                    </div>

                    {isListening && (
                        <motion.p initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="text-center text-red-400 text-sm mt-2">
